...
function renderProjectCard(project) {
  ...
  return `
    <div class="project-card" data-status="${project.status}" data-id="${project.id}">
      <div class="thumb" style="background:${project.coverGradient};">${project.name}</div>
      <div class="body">
...
function renderHeroArt(project) {
  ...
  return `
    ...
    <div style="background:${project.coverGradient}; border-radius:12px; padding:38px 20px; text-align:center; color:var(--cream);">
      <div style="font-family:var(--font-display); font-weight:800; font-size:2rem; text-shadow:0 3px 0 var(--red-dark);">${project.name}</div>
      <div style="font-family:var(--font-display); font-size:0.85rem; letter-spacing:1px; text-transform:uppercase; margin-top:4px; opacity:0.9;">${project.description.split('.')[0]}</div>
      ${emojisHtml ? '<div style="margin-top:22px; display:flex; justify-content:center; gap:10px;">' + emojisHtml + '</div>' : ''}
    </div>
